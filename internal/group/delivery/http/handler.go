...
func MapRoutes(r *gin.RouterGroup, db pkgMongo.Database, mw middleware.Middleware) {
	...
	writes.PATCH("/:id", h.update)
	writes.DELETE("/:id", h.remove)
	writes.POST("/:id/join", h.join)
	writes.POST("/:id/posts", h.createPost)
	writes.DELETE("/:id/posts/:postID", h.removePost)
	writes.DELETE("/:id/members/me", h.leave)
	auth.GET("/:id/members", h.members)
	writes.PATCH("/:id/members/:userID", h.updateMember)
}
